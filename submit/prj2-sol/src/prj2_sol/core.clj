  ...
  (if (empty? items)
    ...
    (let [first-item (first items)
      ...
      (if (= category (:category first-item))
        (cons first-item (items-with-category1 rest-of-items category))
        (items-with-category1 rest-of-items category)))))
...
(defn items-total3 [items]
  "return total price of all items"  
  (reduce (fn [acc item] (+ acc (* (:n-units item) (:unit-price item))))
          0
          items))
