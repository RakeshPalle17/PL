...
category_items1(_Category, []) ->
    [];
category_items1(Category, [X|Xs]) ->
    if
        X#order_item.category =:= Category ->
            [X | category_items1(Category, Xs)]; 
        true ->
            category_items1(Category, Xs)
    end.
...
category_items2(Category, Items) ->
    lists:filter(fun(Item1) -> Item1#order_item.category == Category end, Items).
...
category_items3(Category, Items) ->
    [Item1 || Item1 <- Items, Item1#order_item.category == Category].
...
delete_item(_Sku, Items) ->
    [Item1 || Item1 <- Items, element(2, Item1) =/= _Sku].
...
% [E] appended.
upsert_item(_Item, []) -> [_Item];
upsert_item(Item, [X|Xs]) when X#order_item.sku == Item#order_item.sku ->
    [Item|Xs];
upsert_item(Item, [X|Xs]) ->
    [X|upsert_item(Item, Xs)].
...
find_items(Preds, Items) ->
    [Resp || Resp <- Items, lists:all(fun(Pred) -> Pred(Resp) end, Preds)].
...
items_req({read, Sku}, Items) ->
    case find_items([fun(X) -> X#order_item.sku == Sku end], Items) of
        [Resp] -> {ok, Resp, Items};
        _ -> {err, Sku, Items}
    end;
	
items_req({upsert, ItemsZ}, Items) ->
    {ok, void, upsert_item(ItemsZ, Items)};

	
items_req(_, Items) ->
    {err, "The request is Invalid", Items}.
...
items_req_with_sort({sort}, Items) ->
    Sort_Items = lists:sort(fun(X, Y) -> X#order_item.sku =< Y#order_item.sku end, Items),
    {ok, void, Sort_Items};
items_req_with_sort(Request, Items) ->
    items_req(Request, Items).
...
start_items_server(Items, Func) ->
    Serv = spawn(fun() -> item_server_loop(Items, Func) end),
    register(items, Serv),
    {ok, Serv}.
...
stop_items_server() ->
    items ! {self(), stop},
    receive
        {items, {ok, stopped}} -> {ok, stopped}
    end.


% send request Request to server registered under ID items and return 
% Result from server.
items_client(Request) ->
    items ! {self(), Request},
    receive
        {items, Resp} -> Resp
    end.
...
item_server_loop(Items, Func) ->
    receive
        {Cpid, stop} ->
            Cpid ! {items, {ok, stopped}};

        {Cpid, {new_fn, Fn1}} ->
            Cpid ! {items, {ok, void}},
            item_server_loop(Items, Fn1);
			
        {Cpid, Request} ->
            {Sts, Resp, Items1} = apply(Func, [Request, Items]),
            Cpid ! {items, {Sts, Resp}},
            item_server_loop(Items1, Func)
    end.
